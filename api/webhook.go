...
func InitWebhook(r *mux.Router) {
	...
	mr.Handle("/ephemeral/{id:[A-Za-z0-9]+}/{user:[A-Za-z0-9]+}", ApiAppHandler(createEphemeralPost)).Methods("POST")
}
...
func incomingWebhook(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if !c.HasPermissionsToChannel(pchan, "createIncomingHook") && channel.Type == model.CHANNEL_DIRECT {
...
func createEphemeralPost(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if len(userId) == 0 {
		c.Err = model.NewLocAppError("createEphemeralPost", "web.create_ephemeral_post.user_id.app_error", nil, "")
		return
	} else {
	...
	pchan := Srv.Store.Channel().CheckPermissionsTo(hook.TeamId, channel.Id, userId)
	...
	CreateEphemeralPost(c, userId, channel.Id, text, overrideUsername, overrideIconUrl, parsedRequest.Props, webhookType)
