...
func InitGuest(r *mux.Router) {
	...
	sr.Handle("/invite_info", ApiAppHandler(guestInviteInfo)).Methods("POST")
	...
	if sgresult := <-Srv.Store.Guest().GetByInviteId(id); sgresult.Err != nil {
		...
		if scmresult := <-scm; scmresult.Err != nil {
			...
			if gmresults := <-guestMembers; gmresults.Err != nil {
				...
				if len(userIds) > 0 {
					<-Srv.Store.Guest().RemoveUsers(userIds, model.GetMillis())
					message := model.NewMessage(c.Session.TeamId, "", c.Session.UserId, model.ACTION_GUESTS_CHANGED)
...
func CreateGuest(c *Context, guest *model.Guest, channel *model.Channel, user *model.User) (*model.User, *model.AppError) {
	...
	if result := <-Srv.Store.User().GetByEmail(user.TeamId, user.Email); result.Err != nil {
		if result := <-Srv.Store.User().Save(user); result.Err != nil {
			...
			fullName := model.Preference{UserId: ruser.Id, Category: "display_settings", Name: "name_format", Value: "full_name"}
			if presult := <-Srv.Store.Preference().Save(&model.Preferences{mark, embed, fullName}); presult.Err != nil {
				l4g.Error(utils.T("api.user.create_user.tutorial.error"), presult.Err.Message)
			}

			message := model.NewMessage(ruser.TeamId, "", ruser.Id, model.ACTION_NEW_USER)
			PublishAndForget(message)
			...
			newContext := &Context{mockSession, model.NewId(), "", c.Path, nil, c.teamURLValid, c.teamURL, c.siteURL, c.T, model.DEFAULT_LOCALE}
		...
		if ruser.DeleteAt > 0 {
			ruser.DeleteAt = 0
			if result := <-Srv.Store.User().Update(ruser, true); result.Err != nil {
				return nil, result.Err
			}
		}
		...
		if result := <-Srv.Store.Channel().GetMember(channel.Id, ruser.Id); result.Err != nil {
			...
			<-Srv.Store.Channel().Update(channel)
		}
	}

	message := model.NewMessage(ruser.TeamId, "", ruser.Id, model.ACTION_GUESTS_CHANGED)
	PublishAndForget(message)
...
func guestInviteInfo(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if result := <-Srv.Store.Channel().Get(guest.ChannelId); result.Err != nil {
		c.Err = result.Err
		return
	} else {
		channel = result.Data.(*model.Channel)
	...
	if result := <-Srv.Store.Team().Get(channel.TeamId); result.Err != nil {
		c.Err = result.Err
		return
	} else {
		team = result.Data.(*model.Team)
	...
	if len(c.Session.UserId) == 0 {
		...
		props["team_display_name"] = team.DisplayName
		props["team_name"] = team.Name
		props["team_id"] = team.Id
	} else {
		if c.IsGuestUser() {
			if result := <-Srv.Store.Channel().GetMember(channel.Id, c.Session.UserId); result.Err == nil {
				props["redirect_uri"] = "/" + team.Name + "/guest/" + channel.Name
			}
		} else {
			props["redirect_uri"] = "/" + team.Name + "/channels/" + channel.Name
		}
	}
