...
type InstalledAddonCommand struct {
	Token string `json:"token"`
	Url   string `json:"url"`
}
...
type InstalledAddon struct {
	...
	Commands []InstalledAddonCommand `json:"commands"`
}
...
func installAddon(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	var addonCmds []model.AddonCommand
	if acmdchan := <-Srv.Store.Addon().GetAddonCommands(addon.Id); acmdchan.Err != nil {
		c.Err = acmdchan.Err
		return
	} else {
		addonCmds = acmdchan.Data.([]model.AddonCommand)
	}
	...
	cmds := make([]InstalledAddonCommand, 0)
	for _, acmd := range addonCmds {
		cmd := &model.Command{CreatorId: bot.Id, TeamId: bot.TeamId, Trigger: acmd.Trigger, Method: acmd.Method, Username: acmd.Username, IconURL: acmd.IconURL,
			AutoComplete: acmd.AutoComplete, AutoCompleteDesc: acmd.AutoCompleteDesc, AutoCompleteHint: acmd.AutoCompleteHint, DisplayName: acmd.DisplayName,
			URL: acmd.URL, AddonId: addon.Id}
		if cchan := <-Srv.Store.Command().Save(cmd); cchan.Err != nil {
			c.Err = cchan.Err
			<-Srv.Store.Webhook().PermanentDeleteIncomingByAddon(addon.Id)
			<-Srv.Store.Webhook().PermanentDeleteOutgoingByAddon(addon.Id)
			<-Srv.Store.Command().PermanentDeleteByAddon(addon.Id)
			return
		} else {
			cm := cchan.Data.(*model.Command)
			cmds = append(cmds, InstalledAddonCommand{Token: cm.Token, Url: cm.URL})
		}
		installed.Commands = cmds
	}
...
func uninstallAddon(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if cmdschan := <-Srv.Store.Command().GetByTeamAddon(teamId, addonId); cmdschan.Err != nil {
		c.Err = model.NewLocAppError("uninstallAddon", "store.sql_addon.uninstall.app_error", nil, cmdschan.Err.DetailedError)
		return
	} else {
		cmds = cmdschan.Data.([]*model.Command)
	}
	...
	if cmdchan := <-Srv.Store.Command().PermanentDeleteByTeamAddon(teamId, addonId); cmdchan.Err != nil {
		c.Err = model.NewLocAppError("uninstallAddon", "store.sql_addon.uninstall.app_error", nil, cmdchan.Err.DetailedError)
		for _, in := range incoming {
			<-Srv.Store.Webhook().SaveIncoming(in)
		}
