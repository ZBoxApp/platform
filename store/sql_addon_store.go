...
func NewSqlAddonStore(sqlStore *SqlStore) AddonStore {
	...
	for _, db := range sqlStore.GetAllConns() {
		...
		//AddonCommand
		tablecmd := db.AddTableWithName(model.AddonCommand{}, "AddonCommands").SetKeys(false, "Id")
		tablecmd.ColMap("Id").SetMaxSize(26)
		tablecmd.ColMap("AddonId").SetMaxSize(26)
		tablecmd.ColMap("Trigger").SetMaxSize(128)
		tablecmd.ColMap("URL").SetMaxSize(1024)
		tablecmd.ColMap("Method").SetMaxSize(1)
		tablecmd.ColMap("Username").SetMaxSize(64)
		tablecmd.ColMap("IconURL").SetMaxSize(1024)
		tablecmd.ColMap("AutoCompleteDesc").SetMaxSize(1024)
		tablecmd.ColMap("AutoCompleteHint").SetMaxSize(1024)
		tablecmd.ColMap("DisplayName").SetMaxSize(64)
	}
...
func (s SqlAddonStore) CreateIndexesIfNotExists() {
	...
	s.CreateIndexIfNotExists("idx_addon_command_addon_id", "AddonCommands", "AddonId")
}
	...
	go func() {
		...
		if transaction, err := s.GetMaster().Begin(); err != nil {
			...
			addon = &model.Addon{
				...
				EnableIncomingWebhook: descriptor.Webhooks.EnableIncoming,
			...
			if result.Err == nil {
				...
				for _, hook := range descriptor.Webhooks.Outgoing {
				...
				for _, compact := range descriptor.Compacts {
				...
				for _, panel := range descriptor.Webpanels {
				...
				for _, dialog := range descriptor.Dialogs {
				...
				for _, cmd := range descriptor.Commands {
					ac := &model.AddonCommand{AddonId: addon.Id, DisplayName: cmd.DisplayName, Trigger: cmd.Trigger, Method: cmd.Method, Username: cmd.Username,
						IconURL: cmd.IconURL, AutoComplete: cmd.AutoComplete, AutoCompleteDesc: cmd.AutoCompleteDesc, AutoCompleteHint: cmd.AutoCompleteHint,
						URL: cmd.URL}
					if upsertResult := s.saveCommandT(transaction, ac); upsertResult.Err != nil {
						result = upsertResult
						break
					}
				}
...
func (s SqlAddonStore) saveCommandT(transaction *gorp.Transaction, cmd *model.AddonCommand) StoreResult {
	result := StoreResult{}

	if len(cmd.Id) > 0 {
		result.Err = model.NewLocAppError("SqlAddonStore.Save", "store.sql_addon.save.existing_addon_command.app_error", nil, "id="+cmd.Id)
		return result
	}

	cmd.PreSave()
	if result.Err = cmd.IsValid(); result.Err == nil {
		if err := transaction.Insert(cmd); err != nil {
			result.Err = model.NewLocAppError("SqlAddonStore.Save", "store.sql_addon.save.addon_command.app_error", nil, "id="+cmd.Id+", "+err.Error())
		}
	}
...
func (s SqlAddonStore) GetAddonCommands(addonId string) StoreChannel {
	storeChannel := make(StoreChannel)

	go func() {
		result := StoreResult{}
		var data []model.AddonCommand

		_, err := s.GetReplica().Select(&data, "SELECT * FROM AddonCommands WHERE AddonId = :AddonId", map[string]interface{}{"AddonId": addonId})
		if err != nil {
			result.Err = model.NewLocAppError("SqlAddonStore.GetAddonCommands", "store.sql_addon.get.addon_commands.app_error", nil, "err="+err.Error())
		} else {
			result.Data = data
		}
...
func (s SqlAddonStore) PermanentDeleteAddon(addonId string) StoreChannel {
	...
	go func() {
		...
		if transaction, err := s.GetMaster().Begin(); err != nil {
			...
			if result.Err == nil {
				result = s.permanentDeleteAddonCommandsT(transaction, addonId)
			}
...
func (s SqlAddonStore) permanentDeleteAddonCommandsT(transaction *gorp.Transaction, addonId string) StoreResult {
	result := StoreResult{}

	_, err := transaction.Exec("DELETE FROM AddonCommands WHERE AddonId = :AddonId", map[string]interface{}{"AddonId": addonId})

	if err != nil {
		result.Err = model.NewLocAppError("SqlAddonStore.permanentDeleteAddonWebhooksT", "store.sql_addon.delete.addon_commands.app_error", nil, "addon_id="+addonId+", err="+err.Error())
	}
